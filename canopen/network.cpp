...
Result<void> Network::add_node(std::unique_ptr<Node> node) {
    ...
    if (initialized_ || this->node(id)) return Error{ErrorCode::InvalidState};
    nodes_.push_back(std::move(node));
    return {};
}
Result<void> Network::add_slave_node(std::unique_ptr<Node> node) {
    if (initialized_ || !node || this->node(node->id())) return Error{ErrorCode::InvalidState};
    serves_sdo_[node->id()] = true;
    nodes_.push_back(std::move(node));
    return {};
}
Result<void> Network::initialize() {
    ...
    for (auto& n : nodes_) {
        ...
        for (std::size_t i = 0; i < n->rpdos().size(); ++i) {
            r = routes_.add(
...
Result<void> Network::start() noexcept {
    if (!initialized_) return Error{ErrorCode::InvalidState};
    const auto now = std::chrono::steady_clock::now();
    for (const auto& node : nodes_) node->arm_heartbeat_monitor(now);
    running_ = true;
    return {};
}
...
Result<void> Network::tick(MonotonicTimestamp now) noexcept {
    for (const auto& n : nodes_) {
        ...
        if (n->should_produce_heartbeat(now)) {
            can::Frame frame{};
            frame.id = 0x700 + n->id();
            frame.size = 1;
...
Result<void> Network::send_synchronous_tpdos() noexcept {
    for (const auto& n : nodes_)
        for (std::size_t i = 0; i < n->tpdos().size(); ++i)
            if (n->tpdos()[i].transmission_type() >= 1 &&
                n->tpdos()[i].transmission_type() <= 240 &&
                sync_count_ % n->tpdos()[i].transmission_type() == 0) {
                can::Frame frame;
...
Result<void> Network::dispatch(const can::Frame& f, MonotonicTimestamp now) noexcept {
    ...
    if (route->kind == RouteKind::Nmt) {
        ...
        for (auto& n : nodes_)
            if (target == 0 || n->id() == target) n->receive_nmt(command);
        return {};
    }
    ...
    if (route->kind == RouteKind::SdoRequest) {
        ...
        sdo_server_by_node_[n->id()]->process(f, response);
        auto tx = send(response);
        return tx;
    }
