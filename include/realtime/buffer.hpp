...
class Buffer final {
    ...
    static_assert(
        std::is_trivially_copy_assignable_v<T>, "Buffer payload must be trivially copy assignable");
    static_assert(
        std::is_trivially_destructible_v<T>, "Buffer payload must be trivially destructible");
    static_assert(
        std::is_nothrow_copy_constructible_v<T>, "Buffer payload copy construction must not throw");
    static_assert(
        std::is_nothrow_copy_assignable_v<T>, "Buffer payload copy assignment must not throw");
    static_assert(
...
public:
    ...
    void write(const T& value) noexcept {
        ...
        const std::uint32_t previous = published_slot_.exchange(
...
private:
    ...
    [[nodiscard]] Slot* slot_address(std::uint32_t slot) noexcept { return &slots_[slot]; }
