...
[[nodiscard]] bool checked_difference(Rep later, Rep earlier, Duration& result) noexcept {
    if (later < earlier || (earlier < 0 && later > std::numeric_limits<Rep>::max() + earlier)) {
        return false;
    }
...
[[nodiscard]] std::uint64_t saturating_add(
    std::uint64_t value, std::uint64_t increment, bool& saturated) noexcept {
...
void TimingStatistics::add_sample(
    TimingStatistics::DurationAccumulator& accumulator, std::uint64_t sample_count, Duration sample,
...
DurationSummary TimingStatistics::summary(
    const TimingStatistics::DurationAccumulator& accumulator, std::uint64_t sample_count) noexcept {
    if (sample_count == 0) {
        return {};
    }
...
bool TimingStatistics::try_observe(const PeriodicWait& wait, TimePoint completion_time) noexcept {
    if (wait.period <= Duration::zero()) {
        return false;
    }
    ...
    if (!checked_difference(
            ...
            wait.scheduled_time.time_since_epoch().count(), release_latency) ||
        release_latency != wait.lateness ||
        !checked_difference(
            completion_time.time_since_epoch().count(), wait.wake_time.time_since_epoch().count(),
            execution_time) ||
        !checked_add(
            wait.scheduled_time.time_since_epoch().count(), wait.period.count(),
            next_deadline_count)) {
        return false;
    }
    ...
    Duration overrun{};
    if (completion_time > next_deadline && !checked_difference(
                                               completion_time.time_since_epoch().count(),
                                               next_deadline.time_since_epoch().count(), overrun)) {
        return false;
    }
    ...
    missed_release_count_ = saturating_add(missed_release_count_, wait.missed_releases, saturated);
