...
class Queue final {
    ...
    static_assert(
        std::is_trivially_copy_assignable_v<T>, "Queue payload must be trivially copy assignable");
    static_assert(
        std::is_trivially_destructible_v<T>, "Queue payload must be trivially destructible");
    static_assert(
        std::is_nothrow_copy_constructible_v<T>, "Queue payload copy construction must not throw");
    static_assert(
        std::is_nothrow_copy_assignable_v<T>, "Queue payload copy assignment must not throw");
    static_assert(
