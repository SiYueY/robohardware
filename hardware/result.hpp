...
namespace common {
    ...
    class [[nodiscard]] Result final {
        static_assert(!std::is_void_v<T>,
                      "Use Result<void, E> for void success values");
        ...
        static_assert(std::is_nothrow_destructible_v<E>,
                      "E must have a noexcept destructor");
    ...
    public:
         ...
         */
        [[nodiscard]]
        static Result success(const T& value) noexcept(std::is_nothrow_copy_constructible_v<T>) {
         ...
         */
        [[nodiscard]]
        static Result failure(const E& error) noexcept(std::is_nothrow_copy_constructible_v<E>) {
         ...
         */
        [[nodiscard]]
        static Result failure(E&& error) noexcept(std::is_nothrow_move_constructible_v<E>) {
    ...
    private:
        ...
        [[nodiscard]]
        E& error_unchecked() noexcept {
            return *std::launder(&storage_.error);
        }

        [[nodiscard]]
        const E& error_unchecked() const noexcept {
            return *std::launder(&storage_.error);
        }
    ...
    private:
        ...
        explicit Result(ValueTag) noexcept
            : state_(State::Value) {
        }
