...
Result get_termios(int fd, termios& attributes) noexcept {
    return captured(::tcgetattr(fd, &attributes));
}
...
Result read_bytes(int fd, void* data, std::size_t size) noexcept {
    return captured(::read(fd, data, size));
}
Result write_bytes(int fd, const void* data, std::size_t size) noexcept {
