...
public:
    ...
    [[nodiscard]] bool try_observe(const PeriodicWait& wait, TimePoint completion_time) noexcept;
...
private:
    ...
    static void add_sample(
        DurationAccumulator& accumulator, std::uint64_t sample_count, Duration sample,
        bool& saturated) noexcept;
    [[nodiscard]] static DurationSummary summary(
        const DurationAccumulator& accumulator, std::uint64_t sample_count) noexcept;
};
